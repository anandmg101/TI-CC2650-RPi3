...
class SensorTag:
    ...
    def char_write_cmd( self, handle, value ):
        ...
        cmd = 'char-write-cmd 0x%02X %X' % (handle, value)
        #print "DEBUGGING:" + cmd
        self.con.sendline( cmd )
        time.sleep(1) #delay for 1 second so that Tag can enable registers
        return

    def char_write_cmd_accel( self, handle, value_code, value_G):
        #print ('char-write-cmd {0:#X} {1:#X}:0{2}'.format(handle, value_code, value_G))
        # The 0%x for value is VERY naughty!  Fix this!
        cmd = ('char-write-cmd 0x{0} {1}:0{2}'.format(handle, value_code, value_G))
        print cmd
        #print "DEBUGGING:" + cmd
        self.con.sendline( cmd )
        time.sleep(1) #delay for 1 second so that Tag can enable registers
