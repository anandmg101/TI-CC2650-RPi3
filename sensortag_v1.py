...
#
# author: anandmg101
# Adpated from Orestis Evangelatos, orestevangel@gmail.com, https://github.com/OrestisEv/SensorTag-Pi3
#
# Notes:
...
reading_iterations = 10 #number of iterations to read data from the TAG
...
def gryo(data):
	if int(data,16) <32768:
		return (int(data,16))
	else:
		return (int(data,16)-65536)
		
def accelRead(raw_data, G):
	gryoX = raw_data.split()[-18] + raw_data.split()[-17]
	gryoY = raw_data.split()[-16] + raw_data.split()[-15]
	gryoZ = raw_data.split()[-14] + raw_data.split()[-13]
	print ("gryo X = " + str(float(gryo(gryoX)*500.0/65536.0)))	
	print ("gryo Y = " + str(float(gryo(gryoY)*500.0/65536.0)))	
	print ("gryo Z = " + str(float(gryo(gryoZ)*500.0/65536.0)))
	
	accelX = raw_data.split()[-12] + raw_data.split()[-11]
	accelY = raw_data.split()[-10] + raw_data.split()[-9]
	accelY = raw_data.split()[-8] + raw_data.split()[-7]
...
def main():
    ...
    G =2
    tag.char_write_cmd_accel('3F','3F',G)
    
    count = 0
    while count < reading_iterations:
		print ("")
		time.sleep(1)
		accelRead(tag.char_read_hnd(0x3C, "accel"),G)
		count += 1 
    ...
    print(temp_lum)
    lux_luminance = hexLum2Lux(temp_lum)
